...
import { DirectoryNode, FileMetadata, FileNode } from './models'
...
type ProviderSetupTestCase = {
  ...
  expectedProviderSetup: 'none' | 'full-setup'
}
...
type CallbackTestCase = {
  ...
  expectedArgs: (FileMetadata | string | boolean)[]
}
...
vi.mock('./components/TreeSkeleton', () => ({
  TreeSkeleton: ({ containerRef }: { containerRef: React.RefObject<HTMLDivElement> }) => (
...
describe('FileExplorer', () => {
  ...
  describe('Provider Setup Logic', () => {
    providerSetupMatrix.forEach(({ name, hasExistingFileExplorerConfig, expectedProviderSetup }) => {
      it(`given ${name}, when component mounts, expect ${expectedProviderSetup}`, () => {
        ...
        // EXPECT
        if (expectedProviderSetup === 'full-setup') {
