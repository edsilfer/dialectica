...
import { LoadMoreLinesResult, Range } from '../../../models/LineExtensions'
...
export class HunkListViewModel {
  ...
  private hunks: Hunk[] = []
  private hunkInfoCache = new Map<LineMetadata, HunkInfo>()
  private readonly displayMode: DisplayType
  private readonly maxLinesToFetch: number
  private _linePairs?: LineMetadata[]
  private readonly parser: LineParser

  constructor(fileDiff: FileDiff, displayMode: DisplayType, maxLinesToFetch: number, linePairs?: LineMetadata[]) {
   ...
   */
  public loadLines(hunkLine: LineMetadata, result: LoadMoreLinesResult, direction: HunkDirection): HunkListViewModel {
   ...
   */
  private buildContextLines(leftLines: Map<number, string>, rightLines: Map<number, string>): DiffLine[] {
    ...
    for (const rightLineNum of allLeftLineNum) {
      ...
      const lineDiff = new DiffLine(leftContent, 'empty', rightLineNum, matchingRightLineNum ?? rightLineNum)
    ...
    for (const rightLineNum of allRightLineNum) {
      ...
      const lineDiff = new DiffLine(newContent, 'empty', null, rightLineNum)
      contextLines.push(lineDiff)
    }
   ...
   */
  public getLoadRange(baseLine: LineMetadata, direction: HunkDirection): RangePair {
  ...
  private computeLinePairs(): LineMetadata[] {
  ...
  private getHunkLinePairs(hunk: Hunk, prevHunk: Hunk | undefined): LineMetadata[] {
    const linePairs: LineMetadata[] = []
  ...
  private buildFooterHunk(lastHunk: Hunk): LineMetadata {
    const hunkLineDiff = new DiffLine(lastHunk.header, 'hunk', null, null)
  ...
  private buildHunkHeader(hunk: Hunk, direction: HunkDirection): LineMetadata {
    const hunkLineDiff = new DiffLine(hunk.header, 'hunk', null, null)
    const hunkLinePair = this.parser.parse([hunkLineDiff], this.fileDiff.language)[0]
