...
describe('UnifiedLineParser', () => {
  ...
  describe('content handling', () => {
    ...
    it('given add line, when processed, expect content only on right side', () => {
      // GIVEN
  ...
  describe('content assignment', () => {
    it('given lines with content, when parsed, expect content correctly assigned', () => {
      // GIVEN
      const lines = [
        createLineDiff('function test() {', 'context', 1, 1),
        createLineDiff('  return true;', 'add', null, 2),
      ...
      // EXPECT
      expect(result[0].contentLeft).toBe('function test() {')
      expect(result[0].contentRight).toBe('function test() {')
      expect(result[1].contentLeft).toBeNull()
      expect(result[1].contentRight).toBe('  return true;')
    })

    it('given different language, when parsed, expect language preserved', () => {
      // GIVEN
      ...
      expect(result[0].contentLeft).toBe('const x = 1;')
      expect(result[0].contentRight).toBe('const x = 1;')
    })
  })
  ...
  describe('inheritance behavior', () => {
    ...
    it('given parser instance, when parse called, expect to use inherited parse method', () => {
      ...
      expect(result[0]).toBeInstanceOf(LineMetadata)
    })
  })
})
