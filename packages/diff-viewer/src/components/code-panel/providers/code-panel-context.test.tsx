...
// UTILITIES
const createCodePanelConfig = createPropsFactory<Partial<CodePanelConfig>>({
  mode: 'unified',
  ignoreWhitespace: false,
  maxFileLines: 1000,
})

const createCodePanelConfigProps = createPropsFactory<CodePanelConfigContextProps>({
...
const TestWrapper: React.FC<{
  ...
  config?: Partial<CodePanelConfig>
  storage?: 'in-memory' | 'local'
}> = ({ children, config, storage }) => {
  const fullConfig = config ? { ...DEFAULT_CODE_PANEL_CONFIG, ...config } : undefined
  return (
    <CodePanelConfigProvider config={fullConfig} storage={storage}>
      {children}
    </CodePanelConfigProvider>
  )
}

const renderCodePanelConfig = async (props: Partial<CodePanelConfigContextProps> = {}) => {
  const testProps = createCodePanelConfigProps(props)
  return renderWithContext(TestWrapper, useCodePanelConfig, testProps as unknown as Record<string, unknown>)
}
...
const configTestCases = [
  {
    name: 'unified mode',
    config: createCodePanelConfig({ mode: 'unified' }) as CodePanelConfig,
    expectedMode: 'unified',
  },
  {
    name: 'split mode',
    config: createCodePanelConfig({ mode: 'split' }) as CodePanelConfig,
    expectedMode: 'split',
  },
  {
    name: 'ignore whitespace enabled',
    config: createCodePanelConfig({ ignoreWhitespace: true }) as CodePanelConfig,
    expectedIgnoreWhitespace: true,
  },
  {
    name: 'custom max file lines',
    config: createCodePanelConfig({ maxFileLines: 500 }) as CodePanelConfig,
...
describe('CodePanelConfigProvider', () => {
  ...
  describe('default configuration', () => {
    test('given no initial config, when rendered, expect default configuration provided', async () => {
      // WHEN
      const getContext = await renderCodePanelConfig()

      // EXPECT
      expect(getContext().config).toEqual(DEFAULT_CODE_PANEL_CONFIG)
    })

    test('given partial initial config, when rendered, expect config merged with defaults', async () => {
      // GIVEN
      const partialConfig = createCodePanelConfig({
        mode: 'split',
        ignoreWhitespace: true,
      }) as CodePanelConfig

      // WHEN
      const getContext = await renderCodePanelConfig({ config: partialConfig })
  ...
  describe('storage behavior', () => {
    test.each(storageTestCases)(
      ...
      async ({ storage }) => {
        ...
        // WHEN
        const getContext = await renderCodePanelConfig({ storage })

        // EXPECT
        if (storage === 'local') {
          expect(mockReadStorageValue).toHaveBeenCalledWith('__code_panel_view_state__')
    ...
    test('given local storage enabled, when config changes, expect storage updated', async () => {
      // GIVEN
      const getContext = await renderCodePanelConfig({ storage: 'local' })

      // WHEN
      act(() => {
        getContext().setConfig({ mode: 'split' })
      })

      // EXPECT
      const lastCall = mockWriteStorageValue.mock.calls[mockWriteStorageValue.mock.calls.length - 1]
      expect(lastCall[0]).toBe('__code_panel_view_state__')
  ...
  describe('configuration updates', () => {
    test.each(configTestCases)(
      ...
      async ({ config, expectedMode, expectedIgnoreWhitespace, expectedMaxFileLines }) => {
        // WHEN
        const getContext = await renderCodePanelConfig({ config })
    ...
    test('given external config changes, when provider updates, expect config synced', () => {
      // GIVEN
      const initialConfig = createCodePanelConfig({ mode: 'unified' })
      const updatedConfig = createCodePanelConfig({ mode: 'split' })

      const { rerender } = render(
        <TestWrapper config={initialConfig}>
          <ContextSpy cb={vi.fn()} useCtx={useCodePanelConfig} />
        </TestWrapper>,
      )
  ...
  describe('theme inheritance', () => {
    ...
    test('given theme in config, when rendered, expect custom theme used', async () => {
      ...
      const config = createCodePanelConfig({ theme: customTheme }) as CodePanelConfig
  ...
  describe('children rendering', () => {
    test('given custom children, when rendered, expect children rendered', () => {
      ...
      // WHEN
      render(<CodePanelConfigProvider {...props} />)
...
describe('useCodePanelConfig', () => {
  ...
  test('given no provider, when hook called, expect error thrown', () => {
    ...
    // WHEN & EXPECT
    expect(() => render(<HookConsumer />)).toThrow('useCodePanelConfig must be used within a CodePanelConfigProvider')
...
describe('useCodePanelSettings', () => {
  test('given provider context, when hook called, expect config and setter returned', () => {
    // GIVEN
    const TestComponent = () => {
      const settings = useCodePanelSettings()
      return (
        <div>
  ...
  test('given config changes, when hook used, expect updated config', () => {
    // GIVEN
    let setConfig: React.Dispatch<React.SetStateAction<CodePanelConfig>>
  ...
  test('given no provider, when hook called, expect error thrown', () => {
    ...
    // WHEN & EXPECT
    expect(() => render(<HookConsumer />)).toThrow('useCodePanelSettings must be used within a CodePanelConfigProvider')
