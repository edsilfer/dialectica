...
export class Hunk {
   ...
   */
  addChanges(newChanges: DiffLine[]): Hunk {
  ...
  static build(rawLines: RawLine[], filePath: string = ''): Hunk {
    ...
    const changes: DiffLine[] = rawLines.map((line: RawLine) => DiffLine.build(line))
